...
def body_section_count_range(target_minutes: int) -> Tuple[int, int]:
    ...
    included in this count.  The final branch keeps the existing 15-minute C2V
    API range usable without letting long outlines grow without bound.
    """
...
def wrap_storyboard_lecture_lines(data: Any, max_chars: int = LECTURE_LINE_MAX_CHARS) -> Any:
    ...
    def split_line(text: str) -> List[str]:
        ...
        while len(remaining) > max_chars:
            ...
            candidates = [
                index + 1 for index, char in enumerate(window)
    ...
    for section in wrapped["sections"]:
        ...
        if isinstance(evidence, list) and all(index in mapping for index in evidence):
            section["evidence_lines_indices"] = [
                new_index for old_index in evidence for new_index in mapping[old_index]
            ]
...
def _normalize_highlight_groups(groups: Any, line_count: int, page_limit: int) -> List[List[int]]:
    if line_count <= 0:
        return []
    if not isinstance(groups, list) or not groups:
        return [[index] for index in range(line_count)]
    ...
    if flattened != list(range(line_count)):
        return [[index] for index in range(line_count)]

    result: List[List[int]] = []
    for group in normalized_groups:
        result.extend(
            group[offset:offset + page_limit]
            for offset in range(0, len(group), page_limit)
        )
    return result


def _split_exact_code_pages(solution_code: str, max_lines: int = 12) -> List[str]:
...
def _closest_exact_code_snippet(snippet: Any, solution_code: str) -> str:
    pages = _split_exact_code_pages(solution_code)
    if not pages:
        return ""
    ...
    candidate = re.sub(r"\n?```\s*$", "", candidate).strip()
    if candidate in solution_code:
        return candidate

    source_lines = solution_code.splitlines(keepends=True)
    candidate_lines = [line.strip() for line in candidate.splitlines() if line.strip()]
    preferred_size = max(1, min(12, len(candidate_lines) or 1))
    best_score = -1.0
    best_snippet = pages[0]
    for size in range(max(1, preferred_size - 2), min(12, len(source_lines)) + 1):
        for start in range(0, len(source_lines) - size + 1):
            exact = "".join(source_lines[start:start + size])
            comparable = "\n".join(line.strip() for line in exact.splitlines() if line.strip())
            score = SequenceMatcher(None, candidate, comparable).ratio()
            if score > best_score:
                best_score = score
                best_snippet = exact
    return best_snippet
...
def normalize_storyboard_payload(data: Any, *, solution_code: str = "") -> Any:
    """Repair deterministic storyboard formatting without changing teaching content."""
    if not isinstance(data, dict) or not isinstance(data.get("sections"), list):
        return data
    normalized = copy.deepcopy(data)
    sections = normalized["sections"]

    # Make highlight groups valid before wrapping so long lecture lines can be
    # split while all dependent indices are remapped consistently.
    for index, section in enumerate(sections):
        ...
        if not isinstance(lines, list):
            continue
        mode = section.get("layout_mode")
        if solution_code:
            mode = "full_code" if index == len(sections) - 1 else (
                mode if mode in {"no_code", "with_code"} else "no_code"
            )
            section["layout_mode"] = mode
        page_limit = 4 if mode in {"with_code", "full_code"} else 8
        section["highlight_groups"] = _normalize_highlight_groups(
            section.get("highlight_groups"), len(lines), page_limit
        )

    normalized = wrap_storyboard_lecture_lines(normalized)
    sections = normalized["sections"]
    for index, section in enumerate(sections):
        if not isinstance(section, dict):
            continue
        ...
        if lines:
            ...
            if isinstance(evidence, list):
                valid_evidence = list(dict.fromkeys(
                    item for item in evidence
                    if isinstance(item, int) and not isinstance(item, bool) and 0 <= item < len(lines)
                ))
            section["evidence_lines_indices"] = valid_evidence or [0]
            section["zpd_check_line_index"] = 0
            bridge = section.get("bridge_line_index")
            if not isinstance(bridge, int) or isinstance(bridge, bool) or bridge < max(0, len(lines) - 2):
                bridge = len(lines) - 1
            section["bridge_line_index"] = min(bridge, len(lines) - 1)

        if not solution_code:
            continue
        if mode == "full_code":
            section["code_snippets"] = _split_exact_code_pages(solution_code)
        elif mode == "with_code":
            snippets = section.get("code_snippets")
            if not isinstance(snippets, list) or not snippets:
                snippets = [""]
            section["code_snippets"] = [
                _closest_exact_code_snippet(snippet, solution_code)
...
def validate_outline(
    ...
    *,
    ...
    if sections and not (target_seconds * 0.8 <= total_seconds <= target_seconds * 1.2):
        errors.append(f"大纲预计总时长 {total_seconds:.0f} 秒，应在目标 {target_seconds} 秒的 80%-120% 内")
    return data, errors
...
def validate_storyboard(
    ...
    *,
    ...
    for index, section in enumerate(sections):
        ...
        if layout_mode == "full_code":
            full_code_indices.append(index)
        ...
        if solution_code:
            for snippet_index, snippet in enumerate(snippets):
                ...
                elif layout_mode == "full_code" and len(snippet.splitlines()) > 12:
                    errors.append(f"{path}.code_snippets[{snippet_index}] 超过完整代码页 12 行上限")
            if layout_mode == "full_code" and all(isinstance(item, str) for item in snippets):
                if "".join(snippets) != solution_code:
                    errors.append(f"{path}.code_snippets 必须按顺序拼接为逐字完整标准答案")
                for snippet_index, snippet in enumerate(snippets[:-1]):
                    if not snippet.endswith(("\n", "\r")):
                        errors.append(
                            f"{path}.code_snippets[{snippet_index}] 必须在完整源码行结束处分页"
                        )

    if solution_code and full_code_indices != [len(sections) - 1]:
        errors.append("必须且只能由最后一节使用 full_code 完整展示标准答案")

    target_seconds = target_minutes * 60
    if sections and not (target_seconds * 0.8 <= total_seconds <= target_seconds * 1.2):
        errors.append(f"分镜预计总时长 {total_seconds:.0f} 秒，应在目标 {target_seconds} 秒的 80%-120% 内")
    return data, errors
