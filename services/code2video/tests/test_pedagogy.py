...
def _storyboard(solution_code, minutes=5):
    ...
    storyboard["sections"][-1]["layout_mode"] = "full_code"
    return storyboard
...
def test_manual_duration_skips_ai_and_bounds_are_strict():
    ...
    assert calls == []
    assert validate_requested_duration(15, 5, 15) == 15
...
def test_storyboard_normalization_repairs_model_format_without_rewriting_solution():
    solution = (
        "class Solution:\n"
    ...
    first["highlight_groups"] = [[0, 1, 2, 3, 4]]
    first["code_snippets"] = ["```python\nif complement in hashmap:\n    return result\n```"]
    storyboard["sections"][1]["highlight_groups"] = [[], [], []]
    storyboard["sections"][1]["code_snippets"] = [""]

    normalized = normalize_storyboard_payload(storyboard, solution_code=solution)
    _, errors = validate_storyboard(
        normalized,
        outline_sections=outline["sections"],
        target_minutes=5,
    ...
    for index, section in enumerate(normalized["sections"]):
        page_limit = 4 if section["layout_mode"] in {"with_code", "full_code"} else 8
        assert [item for group in section["highlight_groups"] for item in group] == list(
            range(len(section["lecture_lines"]))
        )
        assert all(1 <= len(group) <= page_limit for group in section["highlight_groups"])
        if section["layout_mode"] == "with_code":
            assert all(snippet and snippet in solution for snippet in section["code_snippets"])
        if index == len(normalized["sections"]) - 1:
            assert section["layout_mode"] == "full_code"
            assert "".join(section["code_snippets"]) == solution
