...
按六个维度各 0-20 分评分：
1. `element_layout`：文字可读、无遮挡、无重叠、无出界、无旧元素残留；元素不提前出现、不延迟消失，跨步骤公共结构保持固定锚点，且没有逐句字幕或底部字幕框。
...
硬阻断条件：代码与标准答案不一致、无依据结论、未解释的超纲术语、教学脚手架断裂、讲解文字被遮挡、影响阅读的重叠、元素出界、字符渲染失败、画面严重拥挤、旧元素残留、未来步骤元素提前出现、当前步骤元素延迟到后续旁白才消失、无意义的整体布局跳动。任一硬阻断存在时 `is_good_enough` 必须为 false。
